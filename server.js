...
app.get('/api/leaves/export', auth, async (req, res) => { try { if (!isSuperAdmin(req) && !hasPermission(req, 'view_leaves')) return res.status(403).json({ error: 'No permission' }); const r = await query('SELECT * FROM leaves ORDER BY id DESC', []); const leaves = r.rows.map(rowToLeave); const csv = 'Employee ID,Employee Name,Type,From Date,To Date,Days,Status,Applied On,Decided By,Decided At\n' + leaves.map(l => `"${l.empId || ''}","${l.empName || ''}","${l.type || ''}","${l.fromDate || ''}","${l.toDate || ''}","${l.days || 0}","${l.status || ''}","${l.appliedOn || ''}","${l.decidedBy || ''}","${l.decidedAt || ''}"`).join('\n'); res.setHeader('Content-Type', 'text/csv'); res.setHeader('Content-Disposition', `attachment; filename="leaves_${today()}.csv"`); res.send(csv); } catch (e) { res.status(500).json({ error: e.message }); } });

app.get('/api/employees/export', auth, async (req, res) => { try { if (!isSuperAdmin(req) && !hasPermission(req, 'view_employees')) return res.status(403).json({ error: 'No permission' }); const r = await query('SELECT * FROM employees ORDER BY empId', []); const emps = r.rows.map(rowToEmp); const csv = 'Employee ID,Name,Email,Passport,Status\n' + emps.map(e => `"${e.empId || ''}","${e.name || ''}","${e.email || ''}","${e.passportNo || ''}","${e.status || 'Active'}"`).join('\n'); res.setHeader('Content-Type', 'text/csv'); res.setHeader('Content-Disposition', `attachment; filename="employees_${today()}.csv"`); res.send(csv); } catch (e) { res.status(500).json({ error: e.message }); } });
