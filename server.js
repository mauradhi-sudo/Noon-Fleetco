...
app.get('/api/employees/:id', auth, async (req, res) => {
  ...
  const e = rowToEmp(r.rows[0]);
  if (req.user.role === 'employee' && e.id !== req.user.employeeId) return res.status(403).json({ error: 'No permission' });
  res.json(e);
});

app.post('/api/employees', auth, needPerm('manage_employees'), async (req, res) => {
  const id = uid('E');
  const emp = req.body;
  ...
  await audit(req.user.name, 'Employee added: ' + emp.empId);
  res.json({ id });
});

app.patch('/api/employees/:id', auth, needPerm('manage_employees'), async (req, res) => {
  const r = await query('SELECT * FROM employees WHERE id = ?', [req.params.id]);
  if (!r.rows.length) return res.status(404).json({ error: 'Not found' });
  const e = rowToEmp(r.rows[0]);
  Object.assign(e, req.body);
  await query('UPDATE employees SET data = ? WHERE id = ?', [JSON.stringify(e), e.id]);
  await audit(req.user.name, 'Employee updated: ' + e.empId);
  res.json({ message: 'Updated' });
});

app.patch('/api/employees/:id/status', auth, needPerm('manage_employees'), async (req, res) => {
  const r = await query('SELECT * FROM employees WHERE id = ?', [req.params.id]);
  if (!r.rows.length) return res.status(404).json({ error: 'Not found' });
  const e = rowToEmp(r.rows[0]);
  e.status = req.body.status;
  await query('UPDATE employees SET status = ?, data = ? WHERE id = ?', [req.body.status, JSON.stringify(e), e.id]);
  await audit(req.user.name, 'Employee status changed: ' + e.empId);
  res.json({ message: 'Updated' });
});
...
app.delete('/api/employees/:id', auth, superOnly, async (req, res) => {
  ...
  await audit(req.user.name, 'Employee deleted: ' + e.empId);
  res.json({ message: 'Deleted' });
});

app.post('/api/employees/bulk/import', auth, needPerm('manage_employees'), async (req, res) => {
  ...
  await audit(req.user.name, 'Bulk import: +' + added + ', ~' + updated);
...
app.post('/api/payslips/bulk', auth, needPerm('manage_payroll'), async (req, res) => {
  const rows = req.body.rows || [];
  let count = 0;
  for (const row of rows) {
    const id = uid('P');
    await query('INSERT INTO payslips (id, employeeId, month, data) VALUES (?, ?, ?, ?)', [id, row.employeeId, req.body.month, JSON.stringify(row)]);
    count++;
  }
  await audit(req.user.name, 'Payslips generated: ' + count);
  res.json({ message: 'Generated', count });
});
...
app.post('/api/documents', auth, upload.single('file'), async (req, res) => {
  ...
  await query('INSERT INTO documents (id, employeeId, name, type, size, filename, uploadedAt, uploader) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', [id, empId, docName, req.file.mimetype, req.file.size, filename, today(), req.user.name]);
...
app.post('/api/admins', auth, superOnly, async (req, res) => {
  ...
  await audit(req.user.name, 'Admin created: ' + req.body.email);
...
app.put('/api/config', auth, superOnly, async (req, res) => {
...
app.get('/api/dashboard/metrics', auth, needPerm('view_employees'), async (req, res) => {
  ...
  res.json({
    ...
    details: {
      activeAndWorking: activeAndWorking.map(e => ({ id: e.id, empId: e.empId, name: e.name })),
      onLeave: onLeave.map(e => ({ id: e.id, empId: e.empId, name: e.name })),
      inactive: inactive.map(e => ({ id: e.id, empId: e.empId, name: e.name }))
    }
  });
});
