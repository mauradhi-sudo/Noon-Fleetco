...
const DATE_FIELDS = [
  ...
  'employmentContractSentDate', 'employmentContractAcceptedDate', 'employmentOfferRejectedDate',
  'requestForMOHREJODate', 'mohreJOReceivedDate', 'mohreJOSignedDate', 'laborApprovalDate',
  'statusChangeCompletionDate', 'medicalDate', 'medicalInsuranceEnrollmentDate',
  'medicalInsuranceCardReceivedDate', 'eidApplicationDate', 'eresidencyReceivedDate',
  'eidReceivedDate', 'eidDispatchDate', 'passportReceivedDate', 'biometricDate',
  'myzoiCardsRequestDate', 'myzoiCardsDispatchDate', 'tawjeehContractSubmissionDate',
  'laborCardGeneratedDate', 'iloeRegistrationDate', 'travelDate', 'accommodationArrivalDate',
  'drivingSchoolStartDate', 'induction', 'deploymentDate'
];
    ...
    for (let i = 0; i < data.length; i++) {
      try {
        ...
        for (const dateField of DATE_FIELDS) {
          if (emp[dateField]) {
            emp[dateField] = parseExcelDate(emp[dateField]);
          }
        }
        ...
        const id = uid('E');
        await query(
          'INSERT INTO employees (id, empId, name, email, passportNo, status, data) VALUES (?, ?, ?, ?, ?, ?, ?)',
          [id, emp.empId, emp.name, emp.email, emp.passportNo, emp.status || 'Active', JSON.stringify(emp)]
        );
