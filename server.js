...
async function checkOtp(target, code) {
  const res = await query('SELECT * FROM otps WHERE target = ?', [target]);
  if (!res.rows.length) return false;
  if (res.rows[0].expires < Date.now()) return false;
  if (res.rows[0].code !== String(code)) return false;
  await query('DELETE FROM otps WHERE target = ?', [target]);
  return true;
}
...
app.post('/api/auth/employee/login', async (req, res) => {
  ...
  if (!e.email) return res.status(400).json({ error: 'No email on file' });
...
app.post('/api/leaves', auth, async (req, res) => {
  ...
  if (!r.rows.length) return res.status(404).json({ error: 'Employee not found' });
  const e = rowToEmp(r.rows[0]);
  if (e.joiningDate && (Date.now() - new Date(e.joiningDate)) / 864e5 < 182) return res.status(403).json({ error: 'Probation period' });
  const id = uid('L');
  ...
  await notify(e.id, 'pending', 'Leave submitted', 'Your leave is pending approval');
  await audit(e.name, 'Applied for leave');
...
app.patch('/api/leaves/:id/decide', auth, needPerm('approve_leaves'), async (req, res) => {
  ...
  await notify(lv.employeeId, newSt === 'Approved' ? 'approved' : 'rejected', 'Leave ' + newSt, 'Your leave status: ' + newSt);
  await audit(req.user.name, 'Leave ' + req.params.id + ': ' + newSt);
...
app.get('/api/documents', auth, async (req, res) => {
  if (req.user.role === 'employee') {
    ...
    return res.json(r.rows.map(d => ({ id: d.id, employeeId: d.employeeid ?? d.employeeId, name: d.name, type: d.type, filename: d.filename, uploadedAt: d.uploadedat ?? d.uploadedAt })));
  }
  if (!can(req, 'view_documents')) return res.status(403).json({ error: 'No permission' });
  const r = await query('SELECT * FROM documents', []);
  res.json(r.rows.map(d => ({ id: d.id, employeeId: d.employeeid ?? d.employeeId, name: d.name, type: d.type, filename: d.filename, uploadedAt: d.uploadedat ?? d.uploadedAt })));
});
...
init().then(async () => {
  ...
  app.listen(PORT, () => console.log('HRMS running on port ' + PORT));
