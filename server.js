...
const rowToAccommodation = (r) => ({
  id: r.id,
  campName: r.campname ?? r.campName,
  roomNo: r.roomno ?? r.roomNo,
  capacity: +(r.capacity || 0),
  occupied: +(r.occupied || 0),
  status: r.status
});

const rowToAccommodationAssignment = (r) => ({
  id: r.id,
  employeeId: r.employeeid ?? r.employeeId,
  accommodationId: r.accommodationid ?? r.accommodationId,
  bedNo: r.bedno ?? r.bedNo,
...
app.get('/api/accommodations', auth, async (req, res) => {
  try {
    const r = await query(
      'SELECT * FROM accommodations ORDER BY campName, roomNo',
      []
    );

    res.json(r.rows.map(rowToAccommodation));
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});

app.post('/api/accommodations', auth, async (req, res) => {
  try {

    if (!isSuperAdmin(req) &&
        !hasPermission(req,'manage_employees'))
    ...
    await query(
      ...
      (id,campName,roomNo,capacity,occupied,status)
      VALUES (?,?,?,?,?,?)`,
      [
        id,
        req.body.campName,
...
app.post('/api/accommodation-assignments', auth, async (req, res) => {
  ...
  try {
    ...
    const r = rowToAccommodation(room.rows[0]);
    ...
    await query(
      ...
      (id,employeeId,accommodationId,bedNo,checkInDate,status)
      VALUES (?,?,?,?,?,?)`,
      [
        id,
        req.body.employeeId,
...
app.get('/api/accommodation-assignments', auth, async (req, res) => {

  try {

    const r = await query(
      'SELECT * FROM accommodation_assignments WHERE status=?',
      ['Active']
    );

    res.json(
      r.rows.map(rowToAccommodationAssignment)
    );
...
app.post('/api/accommodation-transfer', auth, async (req,res)=>{
  ...
  try {
    ...
    if(current.rows.length){
      ...
      const old = rowToAccommodationAssignment(
        current.rows[0]
      );
