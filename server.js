...
app.get('/api/documents', auth, async (req, res) => { try { if (req.user.role === 'employee') { const empNo = String(req.user.empId || '').toUpperCase(); const r = await query('SELECT * FROM documents', []); const mine = r.rows.filter(d => { const eid = d.employeeid ?? d.employeeId; if (eid === req.user.employeeId) return true; const no = docFolderNo(d.filename).toUpperCase(); return !!no && no === empNo; }); return res.json(mine.map(d => ({ id: d.id, employeeId: d.employeeid ?? d.employeeId, name: d.name, type: d.type, filename: d.filename }))); } const r = await query('SELECT * FROM documents ORDER BY uploadedAt DESC', []); res.json(r.rows.map(d => ({ id: d.id, employeeId: d.employeeid ?? d.employeeId, name: d.name, type: d.type, filename: d.filename }))); } catch (e) { res.status(500).json({ error: e.message }); } });
